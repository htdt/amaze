...
loader.load( 'media/LeePerrySmith.js', (geometry) => {
  ...
  let simplify = new THREE.SimplifyModifier(400);
  let sortedGeometry = simplify.modify(geometry);
  let lp = lowpoly(sortedGeometry, 1000);
  cleanup(lp);
...
function lowpoly(sortedGeometry, t): void {
  let map = sortedGeometry.map;
  let face;
  let geometry = sortedGeometry.clone();

  for (let i = 0; i < geometry.faces.length; i++) {
      face = geometry.faces[i];
      let oldFace = sortedGeometry.faces[i];
      face.a = oldFace.a;
      face.b = oldFace.b;
      face.c = oldFace.c;
